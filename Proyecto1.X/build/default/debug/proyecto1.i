...
PSECT udata_bank0
  ...
  flag: DS 1
  carga: DS 1
  ; Flag 0 es para la onda triangular
  ;Flag 1 es para la cuadrada
  cuenta: DS 1
  controldisp: DS 1
  cuenta1: DS 1
  prueba: DS 1
...
isr:
    btfsc ((INTCON) and 07Fh), 0 ;chequear bandera de interrupt de puerto b
    call int_iocb
    btfsc ((INTCON) and 07Fh), 2 ;chequear interrupt de tmr0
    call T0_int
...
int_iocb:
    banksel PORTA ;utilizado para frecuencia
...
config_ioc:
    ...
    banksel PORTA
    movf PORTB, W
    bcf ((INTCON) and 07Fh), 0 ;limpiar bandera de interrupt
    return
...
timer0:
    BANKSEL OPTION_REG
    bcf ((OPTION_REG) and 07Fh), 5 ;clock interno
    bcf ((OPTION_REG) and 07Fh), 4 ;edge
    bcf ((OPTION_REG) and 07Fh), 3 ;asignar prescaler a tmr0
    bcf ((OPTION_REG) and 07Fh), 2 ;TMR0 con prescaler 1:256
...
reinicio_tmr0:
    ...
    bcf ((INTCON) and 07Fh), 2 ;limpiar bandera
    return
...
int_enable:
    ...
    bsf ((INTCON) and 07Fh), 3 ;habilitar interrupción del portb
    bcf ((INTCON) and 07Fh), 0 ;bajar bandera
    bsf ((INTCON) and 07Fh), 5 ;habilitar interrupción de tmr0
    bcf ((INTCON) and 07Fh), 2 ;bajar bandera
    bsf ((PIE1) and 07Fh), 0
